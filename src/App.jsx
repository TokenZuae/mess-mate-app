...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState('dashboard'); // 'dashboard' | 'members' | 'expenses' | 'segments' | 'sync' | 'ai' | 'timelines'
  ...
  }, [timelines, activeTimelineId, githubToken, gistId, aiApiKey]);
  ...
  const handleCreateNewTimeline = () => {
    ...
    const newTl = {
      ...
      endDate: '', // open-ended
      status: 'active',
      // Optionally inherit active members with fresh deposit 0
  ...
  const generateAiRoutine = async () => {
    ...
    const contextPayload = {
      ...
      messCycle: { startDate: cycleStartDate, endDate: cycleEndDate || 'Ongoing', totalDays: getDaysInCycle },
      kittyFund: {
        totalDeposits: calculationSummary.totalDeposits,
        paidFromKitty: calculationSummary.paidFromKitty,
        remainingKittyBalance: calculationSummary.remainingKittyBalance
      },
      memberSettlements: calculationSummary.memberStats.map(m => ({
        name: m.name,
        joinedOn: m.joinDate,
        depositPaid: m.deposit,
        outOfPocketPaid: m.directPaid,
        exactExpenseShare: m.calculatedExpenseShare.toFixed(2),
        netStatus: m.netBalance >= 0 ? `Refund Owed $${m.netBalance.toFixed(2)}` : `Needs to Pay $${Math.abs(m.netBalance).toFixed(2)}`
      })),
      spendingLogs: expenses.map(e => ({
        title: e.title,
        amount: e.amount,
        date: e.date,
        paidBy: e.paidBy === 'kitty' ? 'Kitty Fund' : members.find(m => m.id === e.paidBy)?.name || 'Unknown',
        category: e.category
      })),
      totalExpense: calculationSummary.totalMessExpenses
    };

    const prompt = `
    You are an expert financial consultant & mess manager for shared living.
    Analyze this Mess Expense dataset for timeline: "${activeTimeline.name}"
    DATA: ${JSON.stringify(contextPayload)}

    Provide:
    1. Join Date & Timeline Fairness Audit.
    2. Kitty Fund Health & Remaining Balance Guidance.
    3. Custom Meal & Grocery Schedule tailored for current headcount.
    4. Practical Savings & Waste Reduction Advice for future timelines.
    `;
    ...
    const fetchWithBackoff = async (attempt = 0) => {
      ...
      try {
        ...
        if (!textOutput) throw new Error("Received empty response from Gemini API.");
        setAiReport(textOutput);
      } catch (err) {
        if (attempt < delays.length) {
          await new Promise(res => setTimeout(res, delays[attempt]));
          return fetchWithBackoff(attempt + 1);
        }
        setAiError(err.message || 'Failed to communicate with AI API.');
      } finally {
      ...
      <header className="sticky top-0 z-50 bg-[#151B2C]/90 backdrop-blur-md border-b border-slate-800/80 pt-[max(0.75rem,env(safe-area-inset-top))] px-[max(1rem,env(safe-area-inset-left))] pr-[max(1rem,env(safe-area-inset-right))]">
        ...
        <nav className="max-w-md mx-auto flex justify-between space-x-1 overflow-x-auto no-scrollbar pb-2">
          {[
            ...
            { id: 'ai', label: 'AI Advisor', icon: Icons.Sparkles },
          ].map((tab) => {
            const Icon = tab.icon;
          ...
          <div className="space-y-4">
            <div className="bg-slate-850/90 border border-slate-800 rounded-2xl p-4 space-y-3">
              <div className="flex items-center space-x-2">
                ...
                <h2 className="text-xs font-bold uppercase tracking-wider text-slate-200">
                  Gemini AI Advisor ({activeTimeline.name})
                </h2>
              </div>
              ...
              <button
                ...
                className="w-full bg-gradient-to-r from-emerald-500 to-teal-400 hover:from-emerald-400 hover:to-teal-300 text-slate-950 font-bold text-xs py-2.5 rounded-xl transition-all shadow-lg shadow-emerald-500/20 flex items-center justify-center space-x-2"
              >
                {isAiLoading ? (
                  <span>Running AI Timeline Analysis...</span>
                ) : (
                  <>
                    <Icons.Sparkles className="w-4 h-4" />
                    <span>Analyze Timeline & Financial Health</span>
                  </>
                )}
              </button>
            </div>
            ...
            {aiReport && (
              <div className="bg-slate-850/90 border border-slate-800 rounded-2xl p-4 space-y-2">
                <h3 className="text-xs font-bold text-emerald-400 uppercase tracking-wider">AI Audit & Recommendations</h3>
                <div className="text-xs text-slate-300 leading-relaxed whitespace-pre-wrap font-sans bg-slate-900/80 p-3 rounded-xl border border-slate-800">
                  {aiReport}
                </div>
              </div>
            )}
      ...
      <footer className="fixed bottom-0 left-0 right-0 z-50 bg-[#151B2C]/95 backdrop-blur-md border-t border-slate-800/80 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2">
        <div className="max-w-md mx-auto px-4 flex justify-around items-center">
          {[
            ...
            { id: 'timelines', label: 'Timelines', icon: Icons.Calendar },
            { id: 'sync', label: 'GitHub', icon: Icons.Github },
            { id: 'ai', label: 'Advisor', icon: Icons.Sparkles },
          ].map((item) => {
